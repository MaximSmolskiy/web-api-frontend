...
export async function updateEmployeesList(data) {
    ...
    const page = _.clamp(Number(sessionStorage.getItem('page')), 1, Number(sessionStorage.getItem('pagesNumber')));
...
export async function saveEmployee(employee) {
    const token = sessionStorage.getItem('token');
    const response = await fetch(`${baseUrl}/employees`, {method: 'POST', headers: {'Content-Type': 'application/json', 'Authorization': `Bearer ${token}`}, body: JSON.stringify(employee)});
    if (!response.ok) {
...
export async function updateEmployee(employeeId, employee) {
    const token = sessionStorage.getItem('token');
    const response = await fetch(`${baseUrl}/employees/${employeeId}`, {method: 'PUT', headers: {'Content-Type': 'application/json', 'Authorization': `Bearer ${token}`}, body: JSON.stringify(employee)});
    if (!response.ok) {
        if (response.status == 401) {
...
export async function deleteEmployee(employeeId) {
    const token = sessionStorage.getItem('token');
    const response = await fetch(`${baseUrl}/employees/${employeeId}`, {method: 'DELETE', headers: {'Authorization': `Bearer ${token}`}});
